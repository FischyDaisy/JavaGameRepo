...
vec4 calcPointLight(vec4 diffuseC, vec4 specularC, float reflectance, PointLight light, vec3 position, vec3 normal)
{
    vec3 light_direction = light.position - position;
    vec3 to_light_dir  = normalize(light_direction);
    vec4 light_color = calcLightColor(diffuseC, specularC, reflectance, light.color, light.intensity, position, to_light_dir, normal);
...
void main()
{
	...
	fragColor = calcPointLight(diffuseC, specularC, reflectance, pointLight, worldPos.xyz, normal.xyz) * shadowFactor;
}
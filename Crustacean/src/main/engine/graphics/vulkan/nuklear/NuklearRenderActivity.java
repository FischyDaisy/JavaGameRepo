...
public class NuklearRenderActivity {
    ...
    private void createDescriptorSets(SwapChain swapChain) {
        textureDescriptorSetLayout = new DescriptorSetLayout.SamplerDescriptorSetLayout(device, 1, 0, VK_SHADER_STAGE_FRAGMENT_BIT, 0);
    ...
    public void input(Window window) {
        ...
        nk_input_begin(ctx);
        windowInput(window.getKeyboardInput());
    ...
    private void mouseInput(MouseInput mouse) {
        int[] buttonStates = mouse.getButtonStates();
		// Button Input
        for (int i = 0; i < 3; i++) {
            int nkButton = switch (i) {
                ...
                default -> throw new RuntimeException("Unrecognized Mouse Button Input");
            };
            nk_input_button(ctx, nkButton, (int)mouse.getCurrentPos().x, (int)mouse.getCurrentPos().y, buttonStates[i] == GLFW_PRESS);
        }
	...
	private void windowInput(KeyboardInput keyboard) {
        int[] charList = keyboard.getCharList();

        for (int i = 0; i < keyboard.charListLength(); i++) {
            nk_input_unicode(ctx, charList[i]);
        }
        keyboard.clearCharList();

        for (int key = 32; key < KeyboardInput.KEY_STATE_LENGTH; key++) {
            boolean press = keyboard.isKeyPressed(key);
            switch (key) {
                ...
                case GLFW_KEY_LEFT_CONTROL, GLFW_KEY_RIGHT_CONTROL -> {
                    if (press) {
                        nk_input_key(ctx, NK_KEY_COPY, keyboard.isKeyPressed(GLFW_KEY_C));
                        nk_input_key(ctx, NK_KEY_PASTE, keyboard.isKeyPressed(GLFW_KEY_P));
                        nk_input_key(ctx, NK_KEY_CUT, keyboard.isKeyPressed(GLFW_KEY_X));
                        nk_input_key(ctx, NK_KEY_TEXT_UNDO, keyboard.isKeyPressed(GLFW_KEY_Z));
                        nk_input_key(ctx, NK_KEY_TEXT_REDO, keyboard.isKeyPressed(GLFW_KEY_R));
                        nk_input_key(ctx, NK_KEY_TEXT_WORD_LEFT, keyboard.isKeyPressed(GLFW_KEY_LEFT));
                        nk_input_key(ctx, NK_KEY_TEXT_WORD_RIGHT, keyboard.isKeyPressed(GLFW_KEY_RIGHT));
                        nk_input_key(ctx, NK_KEY_TEXT_LINE_START, keyboard.isKeyPressed(GLFW_KEY_B));
                        nk_input_key(ctx, NK_KEY_TEXT_LINE_END, keyboard.isKeyPressed(GLFW_KEY_E));
                    } else {
                        nk_input_key(ctx, NK_KEY_LEFT, keyboard.isKeyPressed(GLFW_KEY_LEFT));
                        nk_input_key(ctx, NK_KEY_RIGHT, keyboard.isKeyPressed(GLFW_KEY_RIGHT));

...
vec4 calcLightColor(vec4 diffuseC, vec4 specularC, float reflectance, vec3 light_color, float light_intensity, vec3 position, vec3 to_light_dir, vec3 normal)
{
...
vec4 calcDirectionalLight(vec4 diffuseC, vec4 specularC, float reflectance, DirectionalLight light, vec3 position, vec3 normal)
{
    return calcLightColor(diffuseC, specularC, reflectance, light.color, light.intensity, position, normalize(light.direction), normal);
}
...
void main()
{
    ...
    vec4 specularC = texture(specularText, textCoord);
    ...
    // Directional Light
    vec4 diffuseSpecularComp = calcDirectionalLight(diffuseC, specularC, reflectance, directionalLight, worldPos.xyz, normal.xyz);

...
import main.engine.utility.physUtils.PhysUtils;
...
public class DbvtAabbMm {
	...
	public static boolean Intersect(DbvtAabbMm a, DbvtAabbMm b, Transform xform) {
		...
		PhysUtils.transposeTransform(d0, xform.rotation, d1);

		float[] s0 = new float[] { 0, 0 };
		float[] s1 = new float[2];
		s1[0] = xform.position.dot(d0);
		s1[1] = s1[0];
	...
	public static void Merge(DbvtAabbMm a, DbvtAabbMm b, DbvtAabbMm r) {
		for (int i=0; i<3; i++) {
			if (PhysUtils.getCoord(a.mi, i) < PhysUtils.getCoord(b.mi, i)) {
				PhysUtils.setCoord(r.mi, i, PhysUtils.getCoord(a.mi, i));
			}
			else {
				PhysUtils.setCoord(r.mi, i, PhysUtils.getCoord(b.mi, i));
			}
			
			if (PhysUtils.getCoord(a.mx, i) > PhysUtils.getCoord(b.mx, i)) {
				PhysUtils.setCoord(r.mx, i, PhysUtils.getCoord(a.mx, i));
			}
			else {
				PhysUtils.setCoord(r.mx, i, PhysUtils.getCoord(b.mx, i));
			}
		}
	}
	...
	private void AddSpan(Vector3f d, float[] smi, int smi_idx, float[] smx, int smx_idx) {
		for (int i=0; i<3; i++) {
			if (PhysUtils.getCoord(d, i) < 0) {
				smi[smi_idx] += PhysUtils.getCoord(mx, i) * PhysUtils.getCoord(d, i);
				smx[smx_idx] += PhysUtils.getCoord(mi, i) * PhysUtils.getCoord(d, i);
			}
			else {
				smi[smi_idx] += PhysUtils.getCoord(mi, i) * PhysUtils.getCoord(d, i);
				smx[smx_idx] += PhysUtils.getCoord(mx, i) * PhysUtils.getCoord(d, i);
			}
		}
	}

...
import main.engine.physics.BulletStats;
...
public abstract class AxisSweep3Internal extends BroadphaseInterface {
	...
	protected void quantize(int[] out, Vector3f point, int isMax) {
		...
		PhysUtils.setMax(clampedPoint, worldAabbMin);
		PhysUtils.setMin(clampedPoint, worldAabbMax);
	...
	public void calculateOverlappingPairs(Dispatcher dispatcher) {
		if (pairCache.hasDeferredRemoval()) {
			...
			PhysUtils.quickSort(overlappingPairArray, BroadphasePair.broadphasePairSortPredicate);

			PhysUtils.resize(overlappingPairArray, overlappingPairArray.size() - invalidPair, BroadphasePair.class);
			invalidPair = 0;

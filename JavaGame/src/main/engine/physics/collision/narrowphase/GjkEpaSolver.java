...
import main.engine.utility.physUtils.ArrayPool;
import main.engine.utility.physUtils.ObjectStackList;
...
public class GjkEpaSolver {
	...
	protected class GJK {
		...
		public Vector3f LocalSupport(Vector3f d, /*unsigned*/ int i, Vector3f out) {
			...
			PhysUtils.transposeTransform(d, wrotations[i].getUnnormalizedRotation(quat), tmp);
		...
		public boolean EncloseOrigin() {
			...
			switch (order) {
				...
				case 1: {
					...
					AxisAngle4f rot = new AxisAngle4f(cst2Pi / 3f, tmp);
					tmpQuat.set(rot);
	...
	protected class EPA {
		...
		public Vector3f GetCoordinates(Face face, Vector3f out) {
			...
			face.v[0].w.sub(o, tmp1);
			face.v[1].w.sub(o, tmp2);
			tmp1.cross(tmp2, tmp);
			a[0] = tmp.length();

			face.v[1].w.sub(o, tmp1);
			face.v[2].w.sub(o, tmp2);
			tmp1.cross(tmp2, tmp);
			a[1] = tmp.length();

			face.v[2].w.sub(o, tmp1);
			face.v[0].w.sub(o, tmp2);
			tmp1.cross(tmp2, tmp);
			a[2] = tmp.length();
		...
		public boolean Set(Face f, Mkv a, Mkv b, Mkv c) {
			...
			f.v[2] = c;
			f.mark = 0;
			nrm.mul(1f / (len > 0f ? len : cstInf), f.n);
		...
		public float EvaluatePD(float accuracy) {
			...
			try {
				...
				if (bestface != null) {
					...
					features[0][0].mul(b.x, tmp1);
					features[0][1].mul(b.y, tmp2);
					features[0][2].mul(b.z, tmp3);
					tmp1.add(tmp2, nearest[0]).add(tmp3, nearest[0]);

					features[1][0].mul(b.x, tmp1);
					features[1][1].mul(b.y, tmp2);
					features[1][2].mul(b.z, tmp3);
					tmp1.add(tmp2, nearest[1]).add(tmp3, nearest[1]);
				}
				else {
					failed = true;
				}
